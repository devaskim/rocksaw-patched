...
static int setintsockopt(int socket, int level, int option, int value) {
  return setsockopt(socket, level, option, (char*)&value, sizeof(value));
}
...
static int getintsockopt(int socket, int level, int option) {
  ...
  int result = getsockopt(socket, level, option, (char*)&value, &size);
...
static int gettimeout(int socket, int option) {
  ...
  result = getsockopt(socket, SOL_SOCKET, option, (char*)&value, &size);
...
init_sockaddr_in(JNIEnv *env, struct sockaddr_in *sin, jbyteArray address) {
  ...
  buf = env->GetByteArrayElements(address, NULL);
...
(JNIEnv *env, jclass cls, jobject buffer)
{
  ...
  if(errno) {
    ...
    sbc = env->GetObjectClass(buffer);
    mid =
      env->GetMethodID(sbc, "append",
                          "(Ljava/lang/String;)Ljava/lang/StringBuffer;");
    env->CallObjectMethod(buffer, mid, str);
  }
}
...
(JNIEnv *env, jclass cls, jint socket, jstring device)
{
  ...
  const char *utf = env->GetStringUTFChars(device, NULL);
  int result = setsockopt(socket, SOL_SOCKET, SO_BINDTODEVICE,
                          (void*)utf, env->GetStringLength(device));

  env->ReleaseStringUTFChars(device, utf);
 ...
 jbyteArray destination, jbyteArray source)
{
  ...
  if(result != SOCKET_ERROR) {
    jbyte *buf = env->GetByteArrayElements(source, NULL);
    ...
    env->ReleaseByteArrayElements(source, buf, 0);
  }
 ...
 jbyteArray data, jint offset, jint len, jint family)
{
  ...
  result = recvfrom(socket, (char *)buf+offset, len, 0, NULL, NULL);
 ...
 jbyteArray data, jint offset, jint len, jint family, jbyteArray address)
{
  ...
  result = recvfrom(socket, (char *)buf+offset, len, 0, saddr, &socklen);

  env->ReleaseByteArrayElements(data, buf, 0);

  buf = env->GetByteArrayElements(address, NULL);
  memcpy(buf, addr, addrlen);
  env->ReleaseByteArrayElements(address, buf, 0);
...
getCurrentTime()
{
	...
	t = (((ULONGLONG)tm.dwHighDateTime << 32) | (ULONGLONG)tm.dwLowDateTime) / 10;
	return (jlong)t;
...
Java_com_savarese_rocksaw_net_RawSocket__1_1recvfrom3(
		...
		jlongArray time)
{
  jint result = Java_com_savarese_rocksaw_net_RawSocket__1_1recvfrom2(env, cls, socket, data, offset, len, family, address);
  jlong currentTime[] = {getCurrentTime()};
  env->SetLongArrayRegion(time, 0, 1, currentTime);
  return result;
}
...
Java_com_savarese_rocksaw_net_RawSocket__1_1getCurrentTime(
		JNIEnv *env, jclass cls){
 ...
 jint scope_id)
{
  ...
  result = sendto(socket, (char *)buf+offset, len, 0, saddr, socklen);
